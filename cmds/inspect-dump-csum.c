...
void print_32_bytes_hex_forward(const uint8_t *bytes, const uint8_t length)
{
...
static int btrfs_lookup_extent(struct btrfs_fs_info *info, struct btrfs_path *path, u64 ino, int cow)
{
	...
	while (1)
	{
		...
		total_csums = btrfs_file_extent_num_bytes(leaf, fi) / csum_root->fs_info->sectorsize;
		path->slots[0]++;
		itemnum++;
